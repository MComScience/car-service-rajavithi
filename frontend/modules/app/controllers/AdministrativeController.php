...
class AdministrativeController extends \yii\web\Controller {
    ...
    public function actionReportPdf() {
        ...
        $content = $this->renderPartial('report_pdf', [
            'dataProvider' => $dataProvider,
            'caption' => 'ตารางคิวรถวันนี้',
        ]);
        $pdf = $this->renderPdf($content);
        return $pdf->render();
    }

    public function actionReportHistoryPdf($from_date, $to_date) {
        $from_date = DateConvert::convertToDb($from_date);
        $to_date = DateConvert::convertToDb($to_date);
        $query = (new \yii\db\Query())
                ->select([
                    'tb_destination.destination_id',
                    'tb_destination.destination',
        ...
        $content = $this->renderPartial('report_pdf', [
            'dataProvider' => $dataProvider,
            'caption' => 'ประวัติรายการ ' . DateConvert::convertToDisplay($from_date) . ' - ' . DateConvert::convertToDisplay($to_date),
        ]);
        $pdf = $this->renderPdf($content);
        return $pdf->render();
    }

    private function renderPdf($content) {
        $pdfHeader = [
            'L' => [
                'content' => 'รายงาน' . Yii::$app->name,
                'font-size' => 12,
                'color' => '#333333',
            ],
            'C' => [
        ...
        $pdfFooter = [
            'L' => [
                'content' => Yii::$app->params['hospital-name'],
                'font-size' => 12,
                'font-style' => 'B',
                'color' => '#999999',
            ],
        ...
        $pdf = new Pdf([
            ...
            'filename' => 'รายงาน' . Yii::$app->name . '.pdf',
        ]);
        return $pdf;
    }
